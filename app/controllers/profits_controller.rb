class ProfitsController < ApplicationController
  ...
  def month
    ...
    weekly_sales = sales.unscope(:order).group_by_day_of_week(:date).sum(:amount)
    weekly_expenses = expenses.unscope(:order).group_by_day_of_week(:date).sum(:amount)
  ...
  def year
    ...
    monthly_sales = sales.unscope(:order).group_by_month(:date, format: "%B").sum(:amount)
    monthly_expenses = expenses.unscope(:order).group_by_month(:date, format: "%B").sum(:amount)
    @monthly_profits = {}
